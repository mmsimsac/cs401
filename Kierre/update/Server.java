...
public class Server {
        ...
        try {
            ...
            while (true) {
                ...
                if(input.equals("q")){
                    ...
                    Packet serverQuitRequestP = new Packet();
                    serverQuitRequestP.event_type = 6;

                    //server.outputStream = new ObjectOutputStream(server.getOutputStream());
                    //server.outputStream.writeObject(serverQuitRequestP);

                    //Need to send serverQuitRequestP to clients



                }
                else if(input.equals("p")){
                    server.printVectors(server.connectionList);
                }
    ...
    public void printVectors(ArrayList<Connection> connectionList) {
        System.out.println("----------------");
        for (Connection x : connectionList) {
            System.out.println("peer ID :" + x.peerID);
            System.out.print("FILE_VECTOR :" );
            for(char y : x.FILE_VECTOR){
                System.out.print(y);
            }
            System.out.println();
            System.out.println("----------------");
